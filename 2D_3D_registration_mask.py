...
class TwoD_ThreeD_Registration:
    ...
    def load_images(self, ct_path: str, ultrasound_path: str, ct_mask_path: Optional[str] = None):
        """
        加载3D CT和2D超声图像
    ...
    def compute_mutual_information(
        ...
        try:
            # 转换为numpy数组
            arr1 = sitk.GetArrayFromImage(image1).flatten()
            arr2 = sitk.GetArrayFromImage(image2).flatten()
            ...
            if mask1 is not None:
                mask1_arr = sitk.GetArrayFromImage(mask1).flatten()
                valid_mask = valid_mask & (mask1_arr > 0)
            
            # 应用mask2（超声mask，如果有）
            if mask2 is not None:
                mask2_arr = sitk.GetArrayFromImage(mask2).flatten()
                valid_mask = valid_mask & (mask2_arr > 0)
            
            # 移除背景像素（HU值过低）
            valid_mask = valid_mask & (arr1 > -500) & (arr2 > -500)
            
            # 检查有效像素数量
            n_valid = np.sum(valid_mask)
            if n_valid < 100:
                print(f"    ⚠️ 警告：有效像素过少 ({n_valid})，可能mask不重叠")
                return -1000.0
            
            # 只在mask区域内计算互信息
            arr1_masked = arr1[valid_mask]
            arr2_masked = arr2[valid_mask]
            
            # 归一化到0-255
            arr1_norm = ((arr1_masked - arr1_masked.min()) / 
                        (arr1_masked.max() - arr1_masked.min() + 1e-8) * 255).astype(np.uint8)
            arr2_norm = ((arr2_masked - arr2_masked.min()) / 
                        (arr2_masked.max() - arr2_masked.min() + 1e-8) * 255).astype(np.uint8)
            
            # 计算归一化互信息
            nmi = normalized_mutual_information(arr1_norm, arr2_norm, bins=number_of_bins)
            
            # 可选：打印有效像素比例（仅第一次）
            if not hasattr(self, '_mi_debug_printed'):
                print(f"    有效像素: {n_valid}/{len(arr1)} ({100*n_valid/len(arr1):.1f}%)")
                self._mi_debug_printed = True
    ...
    def objective_function(self, params: np.ndarray) -> float:
        ...
        try:
            ...
            mi = self.compute_mutual_information(
                extracted_slice, self.ultrasound_2d, 
                mask1=mask_slice, mask2=None
            )
...
def main():
    ...
    # ct_mask_path = r"D:\dataset\CT\MM-WHS2017\ct_train\ct_train_1004_label.nii"  # CT心脏mask路径
    output_dir = r"D:\dataset\TEECT_data\registration_results\2d_3d"
    
    registrator.load_images(ct_path, ultrasound_path, ct_mask_path)
