...
class EdgeBasedICPRegistration:
    ...
    def register(
        ...
        dice_scores, mean_dice = self.compute_dice_coefficient(
            fixed_img, 
            resampled_image,
            label_ids=label_ids
        )
        ...
        # 打印结果
        label_names = {1: "左心室", 2: "左心房", 3: "右心房", 4: "右心室"}
        for label_id, dice in dice_scores.items():
            label_name = label_names.get(label_id, f"Label {label_id}")
            print(f"  {label_name} (Label {label_id}): Dice = {dice:.4f}")
        ...
        # 为每个标签单独添加到metrics（方便后续分析）
        for label_id, dice in dice_scores.items():
            metrics[f'dice_label_{label_id}'] = dice
    ...
    def _visualize_registration(
        ...
        print(f"\n✓ 可视化已保存: {vis_path}")
    def compute_landmarks(self, fixed_label_path: str, moving_label_path: str) -> Tuple[np.ndarray, np.ndarray]:
            # 读取标签图
        # moving_img = sitk.ReadImage(moving_label_path)
        # fixed_img_original = sitk.ReadImage(fixed_label_path)
        ...
        print("\n计算Moving图像的质心:")
        moving_centroids = compute_label_centroids(moving_img, label_ids)

        print("\n计算Fixed图像的质心:")
        fixed_centroids = compute_label_centroids(fixed_img, label_ids)

        # 将质心转换为landmark数组（按label_id顺序排列）
        moving_landmarks = []
        fixed_landmarks = []
        ...
        for moving_path in moving_files:
            try:
                ...
                transform, metrics = self.register(
                    fixed_edge_path=fixed_edge_path,
                    moving_edge_path=str(moving_path),
                    ...
                    use_scaling=False  # 建议改为False，只使用刚性变换
                )
...
def compute_label_centroids(label_image: sitk.Image, label_ids: list = None) -> dict:
    ...
    Args:
        label_image: SimpleITK标签图像
        label_ids: 要计算的标签ID列表，如果为None则计算所有非零标签
    
    Returns:
        centroids: 字典 {label_id: (x, y)} 像素坐标
    """
    # 转换为numpy数组
    label_array = sitk.GetArrayFromImage(label_image).squeeze()
    
    # 如果没有指定label_ids，自动检测所有非零标签
    if label_ids is None:
        label_ids = np.unique(label_array)
        label_ids = label_ids[label_ids > 0]  # 排除背景(0)
    ...
    for label_id in label_ids:
        ...
        if np.sum(mask) == 0:
            print(f"  ⚠️ 警告：Label {label_id} 不存在")
            continue
        ...
        centroids[label_id] = (centroid_x, centroid_y)
        
        print(f"  Label {label_id}: 质心 = ({centroid_x:.2f}, {centroid_y:.2f}), 像素数 = {np.sum(mask)}")
...
def main():
    ...
    if len(moving_landmarks) != len(fixed_landmarks):
        print("moving_landmarks和fixed_landmarks点对数量不一致，请检查标签图是否正确")
        return
    transform, metrics = icp_reg.register(
        fixed_edge_path=fixed_label_path,
        moving_edge_path=moving_label_path,
        output_dir=output_dir,
        sampling_method='grid',
        num_points=500,
        max_iterations=100,
    ...
    print(f"\n最终使用的landmark点对数量: {len(moving_landmarks)}")
    print(f"Moving landmarks:\n{moving_landmarks}")
    print(f"Fixed landmarks:\n{fixed_landmarks}")
    ...
    resampled = sitk.Resample(
        sitk.ReadImage(moving_label_path),
        sitk.ReadImage(fixed_label_path),
        transform,
        sitk.sitkNearestNeighbor,
        0.0
    )
