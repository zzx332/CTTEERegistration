...
class TwoD_ThreeD_Registration:
    ...
    def compute_multi_label_distance_map_loss(
        self,
        ct_label_points: Dict[int, np.ndarray],
        us_label_points: Dict[int, np.ndarray],
        label_weights: Optional[Dict[int, float]] = None,
        ...
        for label_id, ct_pts in ct_label_points.items():
            ...
            label_cost = self.compute_edge_distance_map_loss(
                ct_pts, us_pts, grid_spacing=grid_spacing
            )
            w = label_weights.get(label_id, 1.0)

            total_cost += w * label_cost
            total_weight += w
    ...
    def compute_edge_distance_map_loss(
        self,
        edge1: np.ndarray,
        edge2: np.ndarray,
        grid_spacing: float = 1.0
        ...
        """
        基于边缘距离图的loss：
        1) 将两个边缘点集映射到同一个规则网格上，生成二值边缘图
        2) 对二值图做距离变换，得到两张距离图
        3) 对两张距离图做均方误差（MSE）作为loss（越小越好）
        """
        from scipy.ndimage import distance_transform_edt

        # 边缘点太少时直接返回大惩罚
        if edge1.shape[0] < 5 or edge2.shape[0] < 5:
            return 1e6

        # 合并两个点云，确定公共物理范围
        all_pts = np.vstack([edge1, edge2])
        mins = all_pts.min(axis=0)
        maxs = all_pts.max(axis=0)

        # 给边界留一点padding，避免挤在边缘
        padding = 5.0  # mm
        mins = mins - padding
        maxs = maxs + padding

        # 根据物理范围和grid_spacing构造网格大小
        size = ((maxs - mins) / grid_spacing).astype(int) + 1
        h, w = int(size[1]), int(size[0])  # 注意这里顺序：(x,y)->(col,row)
        ...
        def rasterize(points):
            # 物理坐标 -> 网格坐标
            coords = (points - mins) / grid_spacing
        ...
        edge_img1 = rasterize(edge1)
        edge_img2 = rasterize(edge2)

        # 距离变换：对"非边缘"做距离变换，所以用 1 - edge_img
        dist1 = distance_transform_edt(1 - edge_img1) * grid_spacing
        dist2 = distance_transform_edt(1 - edge_img2) * grid_spacing
        ...
        if debug_save_path is not None:
            ...
            # 边缘二值图
            ax0.imshow(edge_img1, cmap="gray")
            ax0.set_title("Edge Map 1")
            ax0.axis("off")

            ax1.imshow(dist1, cmap="viridis")
            ax1.set_title("Distance Map 1")
            ax1.axis("off")

            ax2.imshow(dist2, cmap="viridis")
            ax2.set_title("Distance Map 2")
            ax2.axis("off")
