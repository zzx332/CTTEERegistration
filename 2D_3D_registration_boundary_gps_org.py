            ...
            if improved and n_iter > 0:
                ...
                if len(history) > 0:
                    ...
                    if pattern_direction_norm > 1e-10:
                        ...
                        # 尝试沿模式方向移动
                        x_trial = x + step_size * pattern_direction
                        x_trial = project_to_bounds(x_trial)
        ...
        for icp_iter in range(max_iterations):
            ...
            def correspondence_cost(p):
                ...
                try:
                    ...
                    valid_dists_ct_to_us = dists_ct_to_us[dists_ct_to_us < max_correspondence_dist]
                    
                    if len(valid_dists_ct_to_us) < 5:
                        return 1e6
                    mean_dist_ct_to_us = np.mean(valid_dists_ct_to_us)
                    
                    # 方向2: US点 -> CT点的平均距离
                    tree_ct = cKDTree(ct_pts)
                    dists_us_to_ct, _ = tree_ct.query(us_points_valid)
                    valid_dists_us_to_ct = dists_us_to_ct[dists_us_to_ct < max_correspondence_dist]
                    
                    if len(valid_dists_us_to_ct) < 5:
                        return 1e6
                    mean_dist_us_to_ct = np.mean(valid_dists_us_to_ct)
            ...
            if use_gps:
                ...
                print(f"  使用Powell优化参数（内部迭代{inner_opt_iterations}次）...")
                result = minimize(
                    correspondence_cost,
                    x0=params,
                    method='Powell',
