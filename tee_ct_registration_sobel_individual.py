...
class RigidRegistration2D:
    ...
    def register_by_averaging_labels(
        self,
        fixed_image_path: str,
        moving_image_path: str,
        labels_to_register: List[int],
        output_dir: str = None,
        config: Dict = None
    ) -> Tuple[Optional[sitk.Image], Optional[sitk.Transform]]:
        """
        分别配准每个标签定义的区域，然后平均变换参数。

        Args:
            fixed_image_path: 带有标签的固定图像路径。
            moving_image_path: 带有标签的移动图像路径。
            labels_to_register: 一个包含要分别配准的标签ID的列表, e.g., [1, 2, 3, 4]。
            output_dir: 主输出目录。
            config: 配准参数配置字典。
        ...
        """
        print("=" * 70)
        print("配准流程: 基于多标签平均变换")
        print(f"要配准的标签: {labels_to_register}")
        print("=" * 70)
        ...
        print("  转换超声图像坐标系...")
        self.fixed_image = sitk.Flip(self.fixed_image, flipAxes=[True, True])  # X轴和Y轴都翻转
        self.fixed_image.SetOrigin((0,0))
        print("    已转换为左手坐标系（X轴翻转, Y轴翻转）")
        collected_transforms = []

        for label_id in labels_to_register:
            print(f"\n--- 开始处理标签 {label_id} ---")
            # # 为当前标签创建mask
            # fixed_mask = sitk.BinaryThreshold(self.fixed_image, lowerThreshold=label_id, upperThreshold=label_id, insideValue=1, outsideValue=0)
            # moving_mask = sitk.BinaryThreshold(self.moving_image, lowerThreshold=label_id, upperThreshold=label_id, insideValue=1, outsideValue=0)

            # # 检查mask是否为空
            # fixed_stats = sitk.LabelShapeStatisticsImageFilter()
            # fixed_stats.Execute(fixed_mask)
            # moving_stats = sitk.LabelShapeStatisticsImageFilter()
            # moving_stats.Execute(moving_mask)

            # if fixed_stats.GetNumberOfLabels() == 0 or moving_stats.GetNumberOfLabels() == 0:
            #     print(f"标签 {label_id} 在其中一个或两个图像中不存在, 跳过。")
            #     continue

            # 为每个标签创建一个子目录
            label_output_dir = None
            if output_dir:
                label_output_dir = Path(output_dir) / f"label_{label_id}"
                label_output_dir.mkdir(parents=True, exist_ok=True)

            try:
                # 使用mask进行配准
                _, transform = self.register(
                    fixed_image_path=fixed_image_path, # 传入路径以便函数内部读取
                    moving_image_path=moving_image_path,
                    output_dir=str(label_output_dir) if label_output_dir else None,
                    label_id=label_id,
                ...
                if transform:
                    print(f"✓ 标签 {label_id} 配准成功。")
                    collected_transforms.append(transform)
                else:
                    print(f"✗ 标签 {label_id} 配准失败或返回空变换。")

            except Exception as e:
                print(f"✗ 标签 {label_id} 配准时发生错误: {e}")
        ...
        # 计算平均变换
        print("\n--- 计算所有成功配准的平均变换 ---")
        average_transform = self._average_similarity_transforms(collected_transforms)
        
        print("最终平均变换参数:")
        self._print_transform_parameters(average_transform)
        ...
        if output_dir:
            ...
            # 保存平均变换后的图像
            sitk.WriteImage(resampled_image, str(main_output_path / "registered_image_averaged.nii.gz"))
            
            # 保存平均变换本身
            sitk.WriteTransform(average_transform, str(main_output_path / "average_transform.tfm"))

            # 保存可视化
            self._save_visualization(
                self.fixed_image,
                self.moving_image, # Note: a true "before" would need an initial transform
                resampled_image,
                main_output_path,
                "averaged_registration_result.png"
            )
        ...
        return resampled_image, average_transform

    def _average_similarity_transforms(self, transforms: List) -> sitk.Euler2DTransform:
        """计算多个变换的参数平均值。"""
        if not transforms:
            return None
        
        # 展平所有CompositeTransform
        flattened_transforms = []
        for t in transforms:
            if isinstance(t, sitk.CompositeTransform):
                # 取最后一个变换
                flattened_transforms.append(t.GetNthTransform(t.GetNumberOfTransforms() - 1))
            else:
        ...
        for t in flattened_transforms:
            if isinstance(t, (sitk.Euler2DTransform, sitk.Similarity2DTransform)):
                if center is None:
                    center = t.GetCenter()
                angles.append(t.GetAngle())
                txs.append(t.GetTranslation()[0])
                tys.append(t.GetTranslation()[1])
        ...
        print(f"  平均了 {len(transforms)} 个变换:")
        # print(f"    - 平均缩放: {avg_scale:.4f} (来自: {[f'{s:.3f}' for s in scales]})")
        print(f"    - 平均角度: {np.degrees(avg_angle):.2f}° (来自: {[f'{np.degrees(a):.2f}°' for a in angles]})")
        print(f"    - 平均平移X: {avg_tx:.2f} (来自: {[f'{x:.2f}' for x in txs]})")
        print(f"    - 平均平移Y: {avg_ty:.2f} (来自: {[f'{y:.2f}' for y in tys]})")

        avg_transform = sitk.Euler2DTransform()
        avg_transform.SetCenter(center)
        # avg_transform.SetScale(avg_scale)
        avg_transform.SetAngle(avg_angle)
        avg_transform.SetTranslation([avg_tx, avg_ty])
    ...
    def register(
        ...
        if initial_transform is None:
            ...
            # # initial_transform.SetCenter(moving_center)
            # initial_transform.SetTranslation(translation)
            # initial_transform.SetAngle(0.25)
            
            print(f"  初始平移: ({translation[0]:.2f}, {translation[1]:.2f}) mm")
            print(f"  初始旋转: {np.degrees(initial_transform.GetAngle()):.1f}°")
            # print(f"  初始缩放: {initial_transform.GetScale():.2f}")
            ...
            if show_initial_alignment:
                ...
                plt.close()
        if label_id:
            org_fixed_image = fixed_image
            org_moving_image = moving_image
            fixed_array = sitk.GetArrayFromImage(fixed_image)
            moving_array = sitk.GetArrayFromImage(moving_image)
            fixed_array[fixed_array != label_id] = 0
            moving_array[moving_array != label_id] = 0
            fixed_image = sitk.GetImageFromArray(fixed_array)
            moving_image = sitk.GetImageFromArray(moving_array)
            fixed_image.CopyInformation(org_fixed_image)
            moving_image.CopyInformation(org_moving_image)
    ...
    def _save_visualization(
        ...
        output_path: Path,
        filename: str = "registration_result.png"
    ):
        """保存可视化结果"""
        print("\n7. 生成可视化...")
...
def main():
    ...
    output_dir = r"D:\dataset\TEECT_data\registration_results\patient-1-4_averaged"

    # ===== 要分别配准的标签列表 =====
    # 确保这些ID在你的fixed和moving图像中都存在
    labels_to_register = [1, 2, 3, 4] # 例如: 1=LV, 2=LA
    ...
    try:
        registrator.register_by_averaging_labels(
